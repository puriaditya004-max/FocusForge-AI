...
if (process.env.NODE_ENV === "production") {
  ["CLIENT_URL", "ADMIN_URL"].forEach((key) => {
    if (!process.env[key]) {
      logger.warn(`${key} is not configured; production CORS may block that app.`);
    }
  });
}
...
// Razorpay requires the exact raw webhook body for HMAC verification.
// Mount this before express.json(), hpp, and input sanitization mutate req.body.
app.post(
...
app.use((err, req, res, next) => {
  ...
  res.status(status).json({
    error: exposeMessage ? err.message : "Something went wrong on the server.",
  });
});
