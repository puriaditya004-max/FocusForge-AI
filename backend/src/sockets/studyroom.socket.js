...
const { containsBannedWord, isRateLimited } = require("../utils/moderation");
    ...
    socket.on("send_message", async ({ roomId, message }) => {
      try {
        ...
        if (containsBannedWord(trimmed)) {
          return socket.emit("error_message", {
            error: "Message blocked: please keep the chat respectful.",
          });
        }
        ...
        const saved = await prisma.studyRoomMessage.create({
          data: { userId: socket.userId, roomId, message: trimmed },
        });
