...
const getRoomMessages = async (req, res) => {
  try {
    ...
    const [messages, total] = await Promise.all([
      ...
      prisma.studyRoomMessage.count({ where: { roomId, isDeleted: false } }),
    ]);
...
// ---------------------------------------------------------
// POST /api/studyroom/rooms/:roomId/messages/:messageId/report
// Any room member can flag a message for admin review. Doesn't
// remove/hide the message immediately — that's a deliberate
// admin-only action (see admin.controller.js resolveMessageReport)
// so one report can't be used to silence people.
// ---------------------------------------------------------
const reportMessage = async (req, res) => {
  try {
    ...
    const message = await prisma.studyRoomMessage.findUnique({ where: { id: messageId } });
    if (!message || message.isDeleted) {
      return res.status(404).json({ error: "Message not found." });
    }

    const report = await prisma.studyRoomMessageReport.create({
      data: {
        messageId,
        reportedById: userId,
    ...
    return res.status(201).json({ message: "Message reported for review.", report });
  } catch (err) {
    logger.error("reportMessage error:", err);
    return res.status(500).json({ error: "Failed to report message." });
  }
};

module.exports = { listRooms, createRoom, joinRoom, getRoomMessages, reportMessage };